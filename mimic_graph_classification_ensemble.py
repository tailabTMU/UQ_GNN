...
from models.mimic_models import GraphConvGNN
from utils.gnn import train_heterogeneous as train, test_heterogeneous as test
...
def main(args):
    ...
    try:
        os.makedirs(pjoin(cwd, 'saved_info_graph_classification', 'ensemble_mimic'))
    ...
    for train_index, test_index in skf.split(iid_data, labels):
        ...
        for ensemble_num in range(1, 4):
            ...
            best_model = pjoin(cwd, 'saved_info_graph_classification', 'ensemble_mimic', 'best_model')
            os.makedirs(best_model, exist_ok=True)
            best_model = pjoin(best_model,
                               f"Ensemble_{ensemble_num}_RUN_{split_number}.pkl")
            ...
            with tqdm(range(params['epochs'])) as t:
                for epoch in t:
                    ...
                    ckpt_dir = os.path.join(root_ckpt_dir,
                                            f"Ensemble_{ensemble_num}_RUN_" + str(split_number))
            ...
            with open(pjoin(cwd, 'saved_info_graph_classification', 'ensemble_mimic',
                            f'{model_name}_ensemble_{ensemble_num}_split_{split_number}_test_logit_vals.txt'),
                      'w+') as myfile:
                np.savetxt(myfile, logit_vals)

            with open(
                    pjoin(cwd, 'saved_info_graph_classification', 'ensemble_mimic',
