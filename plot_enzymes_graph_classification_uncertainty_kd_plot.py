...
from utils.metrics import kl_divergence, agreement, weighted_agreement, linear_weight, nonlinear_weight
...
if __name__ == '__main__':
    ...
    for split_number in range(1, 6):
        ...
        for layer in range(1, 4):
            ...
            for idx in range(len(smoothed_probs)):
                kl_value = kl_divergence(smoothed_probs[idx], smoothed_ref_probs[idx])
                # kl_value = kl_divergence(ref_probs[idx], probs[idx])
                layers_kl[layer - 1].append(kl_value)

        test_agreement_values = agreement(layers_kl)
        agreement_vals += test_agreement_values

        layers_weight = [[] for _ in range(1, 4)]
        for layer in range(1, 4):
            layers_weight[layer - 1] = linear_weight(4, 4 - layer, predictions[layer - 1], ref_predictions)
        test_agreement_values = weighted_agreement(layers_weight, layers_kl)
    ...
    agreement_vals = np.array(agreement_vals)
    linear_agreement_vals = np.array(linear_agreement_vals)
    nonlinear_agreement_vals = np.array(nonlinear_agreement_vals)
    lt_20 = nonlinear_agreement_vals < 10

    # Plot the KDE in Color
    plt.figure(figsize=(10, 6))
    sns.kdeplot(agreement_vals[lt_20], label='Disagreement Metric', shade=True)
    sns.kdeplot(linear_agreement_vals[lt_20], label='Weighted Disagreement Metric (Linear Weight)', shade=True)
    sns.kdeplot(nonlinear_agreement_vals[lt_20], label='Weighted Disagreement Metric (Nonlinear Weight)', shade=True)

    # Add labels and legend
    plt.xlabel('Metric Value')
    plt.ylabel('Density')
    plt.title('KDE Plot of Disagreement Metrics')
    plt.legend()
    ...
    plt.savefig(pjoin(cwd, 'data', 'figs', f'enzymes_density_self_kd_metrics.pdf'))
    plt.close()

    # Plot the KDE in Gray-Scale
    plt.figure(figsize=(10, 6))
    sns.kdeplot(agreement_vals[lt_20], label='Disagreement Metric', shade=True, color='silver')
    sns.kdeplot(linear_agreement_vals[lt_20], label='Weighted Disagreement Metric (Linear Weight)', shade=True, color='dimgray')
    sns.kdeplot(nonlinear_agreement_vals[lt_20], label='Weighted Disagreement Metric (Nonlinear Weight)', shade=True, color='black')
    ...
    plt.savefig(pjoin(cwd, 'data', 'figs', f'enzymes_density_self_kd_metrics-gray.pdf'))
