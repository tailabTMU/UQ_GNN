...
def main(args):
    ...
    try:
        os.makedirs(pjoin(cwd, 'saved_info_graph_classification', 'self_distillation_enzymes'))
    ...
    for train_index, test_index in skf.split(iid_data, labels):
        ...
        for layer in range(config['L']):
            with open(pjoin(cwd, 'saved_info_graph_classification', 'self_distillation_enzymes',
                            f'{model_name}_layer_{(layer + 1)}_split_{split_number}_test_logit_vals.txt'),
                      'w+') as myfile:
                np.savetxt(myfile, logit_vals[layer])

            with open(
                    pjoin(cwd, 'saved_info_graph_classification', 'self_distillation_enzymes',
